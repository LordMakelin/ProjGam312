...
void APlayerChar::Tick(float DeltaTime)
{
	...
	playerUI->UpdateStatBars(Health, Hunger, Stamina);
...
{
	FHitResult HitResult; //Set up raytrace to detect object
	FVector StartLocation = PlayerCamComp->GetComponentLocation();
			...
			if (Stamina > 5.0f)
			{
				if (HitResource)
				{
					...
					if (HitResource->totalResource > resourceValue)
					{
						...
						//Update Material Objective and set value
						...
						SetStamina(-5.0f); //Stamina Cost for player action
					}
					else
					{
						HitResource->Destroy();
	...
	else
	{
		...
		//Update and set Build Objective
